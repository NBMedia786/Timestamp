...
// Prompt input removed from UI - will send empty string to use default backend prompt
const promptInput = null;
...
const progressUploadStats = document.getElementById('progressUploadStats');
const progressUploadSpeed = document.getElementById('progressUploadSpeed');
const progressUploadAmount = document.getElementById('progressUploadAmount');
...
let currentXHR = null; // Track current XHR request for aborting
...
function activateCheckpoint(checkpointName) {
  ...
  console.log('Activated checkpoint:', checkpointName);

  

  // Note: Checkpoint progress bar will be updated by setUpload() based on overall progress
...
function setUpload(pct, label, { speed = null, loaded = null, total = null } = {}) {
  ...
  if (pct <= 0 && !progressModal.classList.contains('hidden')) {
    ...
    setTimeout(() => {
      ...
      if (progressUploadStats) progressUploadStats.style.display = 'none';
      if (progressSpinner) progressSpinner.classList.remove('complete');

    }, 300);
  }

  // Handle new real-time upload stats
  if (label && label.toLowerCase().includes('uploading') && speed !== null && loaded !== null && total !== null) {
    const uploadedMB = loaded / (1024 * 1024);
    const totalMB = total > 0 ? (total / (1024 * 1024)) : 0;
    const remainingMB = totalMB > 0 ? Math.max(totalMB - uploadedMB, 0) : 0;

    console.log('Showing upload stats:', { speed, uploadedMB, totalMB, remainingMB });
    
    if (progressUploadStats) {
      progressUploadStats.style.display = 'flex';
      console.log('Upload stats element found and shown');
    } else {
      console.warn('progressUploadStats element not found!');
    }
    
    if (progressUploadSpeed) progressUploadSpeed.textContent = `${speed.toFixed(2)} MB/s`;
    if (progressUploadAmount) {
      if (totalMB > 0) {
        progressUploadAmount.textContent = `${uploadedMB.toFixed(1)} MB / ${totalMB.toFixed(1)} MB (${remainingMB.toFixed(1)} MB left)`;
      } else {
        progressUploadAmount.textContent = `${uploadedMB.toFixed(1)} MB uploaded`;
      }
    }
  } else if (label && label.toLowerCase().includes('downloading')) {
    const downloadedMB = loaded !== null ? (loaded / (1024 * 1024)) : 0;
    if (progressUploadStats) progressUploadStats.style.display = 'flex';
    if (progressUploadSpeed) progressUploadSpeed.textContent = `Downloading…`;
    if (progressUploadAmount) progressUploadAmount.textContent = `${downloadedMB.toFixed(1)} MB downloaded`;
  } else {
    if (progressUploadStats) progressUploadStats.style.display = 'none';
  }
  ...
  // Update checkpoint progress bar to match overall progress
  if (checkpointProgressBar) {
    if (currentCheckpoint) {
      // Map overall progress (0-100%) to checkpoint progress
      // Upload: 0-30%, Process: 30-50%, Analyze: 50-95%, Complete: 95-98%, Finalize: 98-100%
      const checkpoints = ['upload', 'process', 'analyze', 'complete', 'finalize'];
      const currentIndex = checkpoints.indexOf(currentCheckpoint);
      
      // Calculate checkpoint progress based on overall percentage
      let checkpointProgress = 0;
      
      if (cleanPct <= 30) {
        // Upload phase: 0-30% -> 0-20% of checkpoint bar
        checkpointProgress = (cleanPct / 30) * 20;
      } else if (cleanPct <= 50) {
        // Process phase: 30-50% -> 20-40% of checkpoint bar
        checkpointProgress = 20 + ((cleanPct - 30) / 20) * 20;
      } else if (cleanPct <= 95) {
        // Analyze phase: 50-95% -> 40-80% of checkpoint bar
        checkpointProgress = 40 + ((cleanPct - 50) / 45) * 40;
      } else if (cleanPct <= 98) {
        // Complete phase: 95-98% -> 80-90% of checkpoint bar
        checkpointProgress = 80 + ((cleanPct - 95) / 3) * 10;
      } else {
        // Finalize phase: 98-100% -> 90-100% of checkpoint bar
        checkpointProgress = 90 + ((cleanPct - 98) / 2) * 10;
      }
      
      updateCheckpointProgress(Math.min(100, Math.max(0, checkpointProgress)));
      console.log('Checkpoint progress:', { overall: cleanPct, checkpoint: checkpointProgress.toFixed(1), phase: currentCheckpoint });
    } else {
      // No checkpoint active, just mirror overall progress
      updateCheckpointProgress(cleanPct);
    }
  }
  ...
  // Update spinner state (animate -> checkmark when complete)
  if (progressSpinner) {
    const lowerLabel = (label || '').toLowerCase();
    
    // Only show tick when final analysis is 100% complete
    const isFinalComplete = 
      cleanPct >= 100 || 
      lowerLabel.includes('complete ✓') || 
      lowerLabel.includes('everything is ready');
    
    if (isFinalComplete) {
      console.log('Setting spinner to complete state (final):', { pct: cleanPct, label });
      progressSpinner.classList.add('complete');
    } else {
      // Keep spinning during upload, processing, and analysis
      progressSpinner.classList.remove('complete');
    }
  }
}
...
async function handleSubmit(e) {
  ...
  const prompt = ''; // No custom prompt - use default backend prompt
...
async function performAnalysis(prompt, url, file) {
  ...
  try {
    ...
    console.log('Starting XHR /upload...', { hasFile: !!file, hasUrl: !!url });
    updateStep(url ? 'Connecting to server (YouTube)...' : 'Connecting to server (local file)...');

    // --- Start of new XHR Implementation ---
    let streamStartPosition = 0;

    const xhr = new XMLHttpRequest();
    currentXHR = xhr; // Store reference for aborting
    xhr.open('POST', '/upload', true);

    // Track real-time upload progress
    let uploadStartTime = Date.now();
    xhr.upload.onprogress = (event) => {
      if (event.lengthComputable && file) { // Only show for file uploads
        const percentComplete = (event.loaded / event.total) * 100;
        const elapsedSeconds = (Date.now() - uploadStartTime) / 1000;
        const speed = (elapsedSeconds > 0) ? (event.loaded / (1024*1024)) / elapsedSeconds : 0; // MB/s
        ...
        // Update the progress modal with detailed info
        // Cap the upload bar at 30% to leave room for processing
        const displayPercent = 12 + (percentComplete * 0.18); // 12% to 30%
        setUpload(displayPercent, 'Uploading video…', { 
          speed: speed, 
          loaded: event.loaded, 
          total: event.total 
        });
    ...
    xhr.onprogress = (e) => {
      ...
      // Get the new text chunk from the response
      const currentChunk = xhr.responseText.substring(streamStartPosition);
      streamStartPosition = xhr.responseText.length;

      if (currentChunk) {
        resultsPre.textContent += currentChunk;
        ...
        if (!softTimer && progressStatus && (progressStatus.textContent.includes('Streaming') || progressStatus.textContent.includes('Analyzing'))) {
    ...
    xhr.onload = () => {
      console.log('XHR stream finished (onload).');
    ...
    xhr.onerror = () => {
      ...
      if (activityTimer) clearInterval(activityTimer);
      currentXHR = null;
      updateStep('Network error', true, true);
      addConsoleLog(`[Error] Network error: ${xhr.statusText}`);
      showToast(`Failed to connect to server: ${xhr.statusText}`);
      throw new Error(`Network error: ${xhr.statusText}`);
    };
    
    // Handle abort
    xhr.onabort = () => {
      console.log('XHR request aborted by user');
      if (activityTimer) clearInterval(activityTimer);
      if (etaTimer) clearInterval(etaTimer);
      currentXHR = null;
    };
    ...
    xhr.onloadend = async () => {
      console.log('XHR request finished (onloadend). Status:', xhr.status);
      ...
      if (xhr.status === 0) {
        console.log('XHR request was aborted');
        return; // Don't process aborted requests
      }

      if (xhr.status < 200 || xhr.status >= 300) {
          const errorMsg = xhr.responseText.substring(streamStartPosition) || xhr.statusText;
          addConsoleLog(`[Error] Server error ${xhr.status}: ${errorMsg}`);
          showToast(`Server error ${xhr.status}: ${errorMsg}`);
          setUpload(0, 'Idle');
          throw new Error(`Server error ${xhr.status}: ${errorMsg}`);
      }

      // --- Success logic (moved from old 'finally' block) ---
      if (!hasReceivedData && resultsPre.textContent.trim().length === 0) {
      ...
      if (resultsPre.textContent.trim().length > 0) {
          ...
          if (currentVideoFile) {
            ...
            } catch (videoError) { console.warn('Could not reload local video:', videoError); }
          ...
          // Auto-close progress modal after 2 seconds to show results
          setTimeout(() => {
            console.log('Auto-closing progress modal...');
            setUpload(0, 'Idle');
          }, 2000);
      }
...
// Close modal button handler - abort analysis

closeProgressModal?.addEventListener('click', () => {
  
  // Abort ongoing XHR request
  if (currentXHR) {
    console.log('Aborting analysis...');
    try {
      currentXHR.abort();
      currentXHR = null;
      showToast('Analysis aborted');
      updateStep('Analysis aborted by user', true, true);
    } catch (e) {
  ...
  // Clear timers
  if (etaTimer) clearInterval(etaTimer);
  if (window.softTimer) clearInterval(window.softTimer);
...
async function addHistoryItem(analysisText) {
  ...
  const promptText = currentPromptText || ''; // No custom prompt
  ...
  let name;
  if (fileName) {
    // Use the file name, removing the extension
    name = fileName.replace(/\.[^/.]+$/, '');
  } else if (url && (url.includes('youtube.com') || url.includes('youtu.be'))) {
    try {
      // Try to get the YouTube video ID as a title
    ...
    // Fallback to the old method if no file or YouTube URL
    name = extractTitle(analysisText, promptText, fileName, url);
  }
