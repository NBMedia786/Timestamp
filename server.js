...
app.get('/api/share/:id', checkAuth, async (req, res) => {
  try {
    ...
    const analysis = db.prepare(`
      SELECT share_id as id, analysis_text as analysisText, video_url as videoUrl, 
...
app.get('/share/:id', async (req, res) => {
  try {
    ...
    // Query database instead of JSON file
    const analysis = db.prepare(`
      SELECT share_id, analysis_text, video_url, file_name, created_at
      FROM shared_links 
      WHERE share_id = ?
    `).get(id);
  ...
  try {
    ...
    stats.jobs.recent = db.prepare(`
      SELECT job_id, user_google_id, analyzed_by_name, job_name, video_url, file_name, 
             created_at, status, time_taken_ms, error_message
      FROM analysis_jobs 
      ORDER BY created_at DESC 
      LIMIT ? OFFSET ?
    `).all(limit, offset);
    ...
    stats.jobs.page = page;
    stats.jobs.limit = limit;
