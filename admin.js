...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadStats(page = 1) {
    try {
      const res = await fetch(`/api/admin/stats?page=${page}&limit=${JOBS_PER_PAGE}`);
      if (!res.ok) {
      ...
      if (jobsTable) {
        ...
        updateJobsPagination(data.jobs);
      } else {
  ...
  function updateJobsPagination(jobsData) {
    ...
    if (!prevBtn || !nextBtn || !paginationInfo || !jobsData) return;
    
    const page = jobsData.page || 1;
    const totalPages = jobsData.totalPages || 1;
    const total = jobsData.total || 0;
    const limit = jobsData.limit || JOBS_PER_PAGE;
    ...
    paginationInfo.textContent = `Showing ${start}-${end} of ${total} jobs (Page ${page} of ${totalPages})`;
  ...
  const jobsPrevBtn = document.getElementById('jobs-prev-btn');
  const jobsNextBtn = document.getElementById('jobs-next-btn');
  ...
  if (jobsNextBtn) {
    jobsNextBtn.addEventListener('click', () => {
      loadStats(currentJobsPage + 1).then(() => {
        // currentJobsPage will be updated by updateJobsPagination
      });
    });
  }
      ...
      if (deleteBtn) {
        ...
        if (confirm(`Are you sure you want to permanently delete this analysis job?\n\n(Job ID: ${jobId})`)) {
          try {
            ...
            setTimeout(() => {
              if (row) row.remove();
              loadStats(currentJobsPage); // Reload stats to update counts
            }, 300);
